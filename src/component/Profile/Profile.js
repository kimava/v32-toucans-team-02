...
const Profile = ({ submit, uid }) => {
  ...
  const [profileImg, setProfileImg] = useState(
    'https://firebasestorage.googleapis.com/v0/b/booklog-15241.appspot.com/o/Unknown_person.jpg?alt=media&token=76fdd940-398b-44e7-b82f-fbbedefb900c'
  );
  ...
  const getUserData = (userId) => {
    ...
    const dbRef = firebaseDatabase.ref(`users/${userId}`);
    dbRef.on('value', (snapshot) => {
      if (snapshot.exists()) {
        let data = snapshot.val();
        console.log(`I'm data ${data.name}`);
        //setUid(data.uid)
        setName(data.name);
    ...
    if (selected && type.includes(selected.type)) {
      ...
      storageRef.put(selected).on(
        ...
        async () => {
          ...
          setProfileImg(url);
          console.log(
            'firebase profile Imgurl in select file handler :',
            profileImg
          );
        }
      );
  ...
  const isSelected = (genre) => {
    if (bookGenre.length > 0) {
      for (let i in bookGenre) {
        if (bookGenre[i] === genre) return true;
      }
    }
    return false;
  };
        ...
        <div className='element_container'>
          ...
          <div className='book_genre'>
            ...
            <select
              ...
              onChange={selectfavoriteGenreHandler}
            >
              {isSelected('Fiction') ? (
              ...
              {isSelected('Thriller') ? (
                <option selected value='Thriller'>
                  Thriller
                </option>
              ) : (
